...
function drawRouteCanvas(canvas, svg, drawToken) {
    ...
    let bgWidth = routeWidthBase * 2;
    let fgWidth = routeWidthBase;
    let markerRadius = routeWidthBase * 3 + 0.06;
    let textFontSize = routeWidthBase * 3.5 + 0.06;
    let textDy = routeWidthBase * 1.5;
    let animRadius = Math.max(0.03, routeWidthBase * 1.5);
...
function updateDropCounts() {
    if (dropCountsQueued) return;
    dropCountsQueued = true;
...
function updateDropCountsNow() {
    ...
    const counts = {};
    Object.keys(dropCountElements).forEach(color => counts[color] = 0);
    ...
    Object.keys(dropCountElements).forEach(color => {
        const entry = dropCountElements[color];
...
document.querySelectorAll('.tab').forEach(tab => {
    tab.addEventListener('click', (e) => {
        ...
        setTimeout(() => {
            ...
            if (newMode === 'rearrange' && currentMode === 'puzzle') {
                saveCurrentBoardState();
            } else if (currentMode === 'edit' && (newMode === 'puzzle' || newMode === 'rearrange')) {
                saveCurrentBoardState();
            } else if ((currentMode === 'puzzle' || currentMode === 'rearrange') && newMode === 'edit') {
...
function mirrorBoardColors(colors2D) {
    return colors2D.map(row => [...row].reverse());
}

function mirrorRoute(route) {
    return route.map(p => ({ r: p.r, c: COLS - 1 - p.c }));
}
...
document.getElementById('mirror-btn').addEventListener('click', () => {
    ...
    setTimeout(() => {
        savedBoardColors = mirrorBoardColors(savedBoardColors);
        dragRoute = mirrorRoute(dragRoute);

        if (originalBoardColors) {
            originalBoardColors = mirrorBoardColors(originalBoardColors);
        }
...
document.getElementById('reverse-route-btn').addEventListener('click', () => {
    ...
    setTimeout(() => {
        ...
        if (isReversedState) {
            ...
            let postBoard = computeBoardAfterRoute(savedBoardColors, dragRoute);
            dragRoute.reverse();

            for (let r = 0; r < ROWS; r++) {
                for (let c = 0; c < COLS; c++) {
                    savedBoardColors[r][c] = postBoard[r][c];
                }
            }
        } else {
            dragRoute.reverse();
            if (originalBoardColors) {
                for (let r = 0; r < ROWS; r++) {
                    for (let c = 0; c < COLS; c++) {
                        savedBoardColors[r][c] = originalBoardColors[r][c];
                    }
                }
            }
        }
...
document.getElementById('autoplay-btn').addEventListener('click', () => {
    ...
    if (isReversedState && originalBoardColors) {
        for (let r = 0; r < ROWS; r++) {
            for (let c = 0; c < COLS; c++) {
                savedBoardColors[r][c] = originalBoardColors[r][c];
            }
    ...
    let movingOrb = board[startPos.r][startPos.c];
    ...
    function autoPlayStep(time) {
        ...
        movingOrb.style.top = `${(currentR / ROWS) * 100}%`;
        movingOrb.style.left = `${(currentC / COLS) * 100}%`;
        ...
        while (lastIndex < currentIndex) {
            ...
            board[prevPos.r][prevPos.c] = targetEl;
            board[curPos.r][curPos.c] = movingOrb;

            if (targetEl) {
                targetEl.dataset.r = prevPos.r;
                targetEl.dataset.c = prevPos.c;
                targetEl.style.top = `${(prevPos.r / ROWS) * 100}%`;
                targetEl.style.left = `${(prevPos.c / COLS) * 100}%`;
            }
        }
...
function createBoard() {
    ...
    for (let r = 0; r < ROWS; r++) {
        for (let c = 0; c < COLS; c++) {
            ...
            let retries = 0;
            do {
                color = activeSpawnColors[Math.floor(Math.random() * activeSpawnColors.length)];
                retries++;
...
function createOrbElement(color, r, c) {
    let el = orbPool.pop() || document.createElement('div');
    el.__inPool = false;
    el.className = 'orb ' + color;
    el.dataset.color = color;
    el.dataset.r = r;
...
function getGridPos(x, y, clamp = false) {
    ...
    const cellW = cachedCellW || rect.width / COLS;
    const cellH = cachedCellH || rect.height / ROWS;
...
function handleInputStart(e) {
    ...
    cachedCellW = cachedBoardRect.width / COLS;
    cachedCellH = cachedBoardRect.height / ROWS;
        ...
        if (targetPos && (targetPos.r !== currentDragCell.r || targetPos.c !== currentDragCell.c)) {
            ...
            if (currentMode === 'rearrange') {
                ...
                if (lastObj.r !== targetPos.r || lastObj.c !== targetPos.c) {
                    if (dragRoute.length >= 2 &&
                        dragRoute[dragRoute.length - 2].r === targetPos.r &&
                        dragRoute[dragRoute.length - 2].c === targetPos.c) {
                        dragRoute.pop();
                    } else {
                        dragRoute.push({ r: targetPos.r, c: targetPos.c });
                    }
                }
            }
        }
...
function paintOrb(r, c) {
    ...
    if (currentMode === 'edit' && isPainting && lastPaintCellKey === paintKey) {
        return;
    }
...
async function resolveMatches() {
    resolveId++;
    const currentResolveId = resolveId;
    isResolving = true;
...
async function applyGravity(spawnNew, currentResolveId) {
    ...
    let fallColors = activeSpawnColors.filter(color => color !== 'bom');
    ...
    for (let c = 0; c < COLS; c++) {
        ...
        if (spawnNew) {
            for (let r = 0; r < emptyCount; r++) {
                ...
                newSpawnedOrbs.push({ el: newEl, targetTop: `${(r / ROWS) * 100}%` });
                moved = true;
            }
        }
    }
